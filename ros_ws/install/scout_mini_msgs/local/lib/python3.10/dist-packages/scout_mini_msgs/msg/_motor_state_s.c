...
bool scout_mini_msgs__msg__motor_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("scout_mini_msgs.msg._motor_state.MotorState", full_classname_dest, 43) == 0);
  }
  scout_mini_msgs__msg__MotorState * ros_message = _ros_message;
  {  // header
  ...
  {  // current
    ...
    }
    {
      ...
      assert(PyArray_TYPE(seq_field) == NPY_FLOAT64);
      Py_ssize_t size = 4;
      double * dest = ros_message->current;
      for (Py_ssize_t i = 0; i < size; ++i) {
        double tmp = *(npy_float64 *)PyArray_GETPTR1(seq_field, i);
        memcpy(&dest[i], &tmp, sizeof(double));
      }
...
PyObject * scout_mini_msgs__msg__motor_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("scout_mini_msgs.msg._motor_state");
  ...
  {  // current
    ...
    assert(sizeof(npy_float64) == sizeof(double));
    npy_float64 * dst = (npy_float64 *)PyArray_GETPTR1(seq_field, 0);
    double * src = &(ros_message->current[0]);
    memcpy(dst, src, 4 * sizeof(double));
    Py_DECREF(field);
  }
  {  // temperature
